...
**ソースが切れて繋ぎ直された回数**です。差し替えれば、そのカメラを掴んでいたドライバは
必ず一度切れます。画面は状態を 2 秒ごとに読んでいるので、その数が動いたらカメラの
顔ぶれを数え直し、出ている候補も捨てて訊き直します。ブリッジが入れ替わりを知るのは
顔ぶれを数えたときだけなので (`forgetModesIfCamerasChanged`)、画面を開いたままだと
その機会が来ないためです。

数え直すのは UVC を選んでいるときだけで、走っている数え直しに重ねることもしません。
列挙は ffmpeg を起動して遅ければ 15 秒かかるので、切れやすいシリアルのボードのために
払う代金ではありませんし、重ねれば ffmpeg が積み上がります。
