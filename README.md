...
(最大 30 秒)。上限が長いのは初回試行を丸ごと含める必要があるためです
(MJPEG なら dial・TLS・ヘッダー・最初のフレームでそれぞれ 5 秒、UVC なら
ストール 10 秒 + バックオフ + 再エンコードでの再試行 10 秒)。動作するソースなら
フレームが届いた時点で即座に返るので、長くかかるのは失敗するときだけです。ドライバは自力で再接続するため「まだ失敗していない」ことは何の
保証にもならず、フレームだけが動作の証拠だからです。届かなければ元のソースへ
戻します。
