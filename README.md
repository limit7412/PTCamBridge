...
ログ — は保存されますが、**動作中のブリッジは再起動するまで古い値を使い続けます**。
保存後に画面が名前を挙げて伝えます。これらは起動時にしか読まれないためです。たとえば
トレイはメニューを一度だけ、その時点の言語で組み立てるので、動作中に言語を切り替えても
画面の言葉は一つも変わりません。値だけが変わって動きが変わらない状態を作るより、
保存はした・今は効いていない、と言う方が正確です。
...
読まれます。`PUT /api/v1/config` でこれらを変更した場合、**値は保存されますが動作中の
ブリッジには適用されません**。応答の `pending_restart` にその名前が入るので、呼び出し側は
「保存はされたが今は効いていない」と言えます。応答に入っている設定は動作中のものなので、
これらは要求した値ではなく古い値のまま返ります。次回起動で反映されます。

黙って適用したように振る舞わないのは、値が変わったのに動きが変わらない状態を作らない
ためです。たとえばトレイはメニューを一度だけ、その時点の言語が与えたラベルで組み立てる
ので、動作中に `ui.language` を受け入れても画面上の言葉は一つも変わりません。
